...
export default function SetKelas() {
  ...
  // Staging area: { [siswaId]: targetClassName }
  ...
  const sourceStudents = useMemo(() => {
    ...
    let filtered = siswaList.filter(s => s.kelas === sourceClass && s.is_active && !stagedMoves[s.id]);
    if (searchSource) {
  ...
  // Panel Kanan: Group staged moves by target class
  const stagedGroups = useMemo(() => {
    ...
    // Sort groups alphabetically, but put ALUMNI at the end
  ...
  const selectTarget = (siswaId, target) => {
    ...
    setActiveStudentId(null); // Close bubbles
  };
    ...
    <div className="space-y-6 h-[calc(100vh-8rem)] flex flex-col">
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 flex-shrink-0 animate-fade-up">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h2 className="text-2xl font-bold mb-1 text-[#1e293b]">Rombak / Kenaikan Kelas</h2>
            <p className="text-slate-500 text-sm">
              Klik nama siswa untuk memunculkan bubble pilihan kelas tujuan.
            </p>
          </div>
          <div className="flex items-center gap-3">
            {Object.keys(stagedMoves).length > 0 && (
              <span className="text-sm font-semibold text-orange-500 bg-orange-50 px-3 py-1.5 rounded-lg border border-orange-100 animate-pulse">
                {Object.keys(stagedMoves).length} Menunggu
              </span>
            )}
            <button
              onClick={handleSave}
              disabled={isSaving || Object.keys(stagedMoves).length === 0}
      ...
      <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-6 min-h-0 animate-fade-up delay-100">
                          ...
                          >
                            ...
                            <div className="flex flex-wrap gap-1.5">
                              {kelasList.filter(k => {
                                ...
                                // Fallback jika tingkat tidak dikenali
        ...
        {/* PANEL KANAN (Daftar Pindah) */}
        <div className="bg-white border border-slate-200 rounded-2xl flex flex-col overflow-hidden shadow-sm">
          <div className="p-4 bg-emerald-50/50 border-b border-emerald-100 flex flex-col gap-3">
            <label className="text-xs font-bold text-emerald-800 uppercase tracking-wider">Daftar Pemindahan (Staging)</label>
                        ...
                        {group.students.map(siswa => (
                          <div key={siswa.id} className="p-2.5 flex justify-between items-center hover:bg-slate-50 group transition-colors">
                            <div>
                              ...
                              <p className="text-[10px] text-slate-400 font-mono">Asal: {siswa.kelas}</p>
                            </div>
                            <button
