...
export default function AdminFeatures() {
  ...
  const handleSelectAll = (e) => {
    if (e.target.checked) {
      setSelectedItems(features.map(item => item.id));
    } else {
      setSelectedItems([]);
    }
  };
  ...
  const handleBulkDelete = async () => {
    if (!window.confirm(`Yakin ingin menghapus ${selectedItems.length} keunggulan terpilih?`)) return;
    setIsBulkDeleting(true);
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/admin/features/bulk-delete`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token') || sessionStorage.getItem('token')}`,
          'Accept': 'application/json',
  ...
  const handleSubmit = async (e) => {
    ...
    const url = isEditing 
      ? `${import.meta.env.VITE_API_BASE_URL}/api/admin/features/${formData.id}`
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Hapus keunggulan ini?")) {
      await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/admin/features/${id}`, {
        method: 'DELETE',
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl p-6 sm:p-8 shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-slate-100 relative overflow-hidden animate-fade-up">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h2 className="text-2xl sm:text-3xl font-bold mb-2 text-[#1e293b]">Keunggulan (Mengapa Memilih Kami)</h2>
            <p className="text-slate-500 text-sm max-w-xl">
              Kelola daftar keunggulan sekolah untuk ditampilkan pada halaman utama landing page.
            </p>
          </div>
          <div className="flex items-center gap-3">
            <button onClick={() => { setFormData({ id: null, title: '', description: '', icon: '', order: 0 }); setIsModalOpen(true); }} className="bg-slate-800 hover:bg-slate-900 text-white px-4 py-2.5 rounded-xl transition-all shadow-lg shadow-slate-900/20 font-semibold flex items-center gap-2 shrink-0">
      ...
      <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-[0_4px_20px_rgba(0,0,0,0.03)] animate-fade-up delay-100">
        <div className="flex flex-wrap items-center justify-between mb-6 gap-3">
          <div className="flex items-center gap-4">
            <h3 className="text-[16px] font-bold text-[#1e293b] shrink-0">Daftar Keunggulan</h3>
            ...
            <thead>
              <tr className="border-b border-slate-100 text-[11px] font-bold text-slate-400 uppercase tracking-wider">
                <th className="pb-3 pl-4 w-10">
                  <input type="checkbox" className="rounded border-slate-300" checked={features.length > 0 && selectedItems.length === features.length} onChange={handleSelectAll} />
                </th>
                <th className="pb-3 pl-2 w-16">Urutan</th>
                <th className="pb-3 w-16 text-center">Icon</th>
                <th className="pb-3">Judul Keunggulan</th>
              ...
              {features.map(item => (
                <tr key={item.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="py-4 text-center font-bold text-slate-700">
                    <div className="flex justify-center"><i className={`bi ${item.icon || 'bi-star'} text-xl text-blue-600`}></i></div>
                  </td>
              ...
              {features.length === 0 && (
                <tr>
                  <td colSpan="6" className="py-16 text-center">
                    <p className="font-semibold text-slate-500">Belum ada data keunggulan</p>
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="fixed inset-0 z-50 flex p-4 bg-black/50 overflow-y-auto">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md m-auto">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center sticky top-0 bg-white rounded-t-xl z-10">
              <h2 className="text-xl font-bold">{formData.id ? 'Edit Keunggulan' : 'Tambah Keunggulan'}</h2>
              <button onClick={() => setIsModalOpen(false)} className="text-gray-400">✕</button>
            </div>
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium mb-1">Judul Keunggulan</label>
                <input type="text" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} className="w-full border p-2 rounded-lg" required placeholder="Contoh: Akreditasi A" />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Deskripsi Singkat</label>
                <textarea value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} className="w-full border p-2 rounded-lg" rows="3" required placeholder="Deskripsi singkat keunggulan..." />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Class Bootstrap Icon (Opsional)</label>
                <input type="text" value={formData.icon} onChange={e => setFormData({...formData, icon: e.target.value})} className="w-full border p-2 rounded-lg" placeholder="Contoh: bi-award, bi-book" />
                <p className="text-xs text-slate-500 mt-1">Cari referensi icon di: <a href="https://icons.getbootstrap.com/" target="_blank" className="text-blue-500">Bootstrap Icons</a></p>
              </div>
              <div>
