...
export default function AdminFasilitas() {
  ...
  const handleSelectAll = (e) => {
    if (e.target.checked) {
      setSelectedItems(fasilitas.map(item => item.id));
    } else {
      setSelectedItems([]);
    }
  };
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/admin/facilities/bulk-delete`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token') || sessionStorage.getItem('token')}`,
          'Accept': 'application/json',
      ...
      if (response.ok) {
        setSelectedItems([]);
        fetchFasilitas();
        showToast(`${selectedItems.length} fasilitas berhasil dihapus`, 'success');
      } else {
            ...
            <thead>
              <tr className="border-b border-slate-100 text-[11px] font-bold text-slate-400 uppercase tracking-wider">
                <th className="pb-3 pl-4 w-10">
                  <input type="checkbox" className="rounded border-slate-300" checked={fasilitas.length > 0 && selectedItems.length === fasilitas.length} onChange={handleSelectAll} />
                </th>
