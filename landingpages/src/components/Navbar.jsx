  ...
  useEffect(() => {
    const fetchNavItems = async () => {
      try {
        ...
        const response = await fetch(`${baseUrl}/api/public/navbars`, {
          headers: {
            'x-api-key': 'smansa123'
          }
        });
        if (response.ok) {
          setNavItems(await response.json());
        }
      } catch (error) {
        console.error("Failed to fetch navbars", error);
      }
    };
    fetchNavItems();
  }, []);
        ...
        <div className="px-6 flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-6 font-bold text-xs" style={{ fontFamily: "'Inter', sans-serif" }}>
            {navItems.map(item => {
              ...
              const isHashLink = item.url ? item.url.startsWith('#') : false;
              const hrefUrl = isHashLink ? `/${item.url}` : (item.url || '#');
            ...
            {navItems.length === 0 && (
              // Default fallback just in case database is empty or not loaded
              <>
